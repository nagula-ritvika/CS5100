...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.particles:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                wParticles[p] += emissionModel[trueDistance]

            if wParticles.totalCount() == 0:
                self.initializeUniformly(gameState)

            else:
                self.particles = [util.sample(wParticles) for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newP = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            part.append(util.sample(newP))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        allPossible = list(itertools.product(self.legalPositions, repeat= self.numGhosts))
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is not None:
                    ...
                    w *= emissionModels[i][trueDistance]
            wParticles[p] += w
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                newP = getPositionDistributionForGhost(setGhostPositions(gameState,newParticle),
                                                            i,self.ghostAgents[i])
                newParticle[i] = util.sample(newP)
